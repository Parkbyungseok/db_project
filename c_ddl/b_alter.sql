...
# 데이터 및 제약조건과 충돌하지 않는 방향으로만 수정 가능

# 1. 기존컬럼의 타입을 다른 타입으로 변경이 가능할까? - x(데이터 문제 발생)
# but 컬럼안에 아무도 값 x(데이터가 없는 경우)에는 가능 
-- alter table ddl_emp modify emp_name int; 

# 2. 기존컬럼의 크기를 기존보다 작은 값을 변경하는 것이 가능할까? - x(데이터 문제 발생)
# but 작은 값을 더 큰 값으로는 변경 가능
-- alter table ddl_emp modify emp_name varchar(1);

# 3. 컬럼에 이미 null인 데이터가 존재할 때 not null 제약조건 추가 가능할까? - x(데이터 문제 발생)
# but 컬럼안에 null 값이 하나도 없다면 추가가능
-- alter table ddl_emp modify job_code char(3) not null;
-- alter table ddl_emp modify age int not null;

# 4. 컬럼에 중복값이 하나라도 존재할 때 unique 제약조건 걸 수 있을까? - x(데이터 문제 발생)
-- alter table ddl_emp modify age int unique; 

# 5. check 제약조건의 범위를 기존 데이터가 포함되지 않는 범위로 변경할 수 있다? - x
...
alter table ddl_emp rename column job to job_code; -- 오류 방지를 위해 원상태로
...
update ddl_dept set dept_code = 'D99' where dept_code = 'D01';
select * from ddl_emp;
