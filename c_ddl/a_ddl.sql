...
create table ddl_emp(
	...
	# 인조키 : 테이블에 식별할 수 있는 컬럼집합도 없고, 대표성으 지니는 컬럼도 없다면,
	#		 인위적으로 컬럼을 만들어 기본키를 지정
	#		 모든 테이블에 단일컬럼 인조키를 추가 
	#			* 대표성을 지니는 단일컬럼 후보키가 없다면 모든 테이블에 단일컬럼 인조키 추가
	...
	# check : 도메인 무결성을 보장. 도메인 정의에 부합하는 값만 컬럼 추가 가능
	age int check(age >= 0),
	...
	# 참조무결성, 외래기 : 참조하고 있는 테이블의 행을 식별하기 위해 사용
	# 외래키 : 유일성과 최송이 만족되는 컬럼을 지정
	dept_code char(3),
...
values('하명도', 20, '000000-111111', 'D01');
select * from ddl_emp;

-- check 제약조건 age에 음수 x
insert into ddl_emp(emp_name, age, emp_no, dept_code)
values('어선정', 20, '000000-211111', 'D01');
select * from ddl_emp;

-- unique 제약조건
insert into ddl_emp(emp_name, age, emp_no, dept_code)
values('헤더', 20, '000000-211112', 'D01');
select * from ddl_emp;

-- 외래키 제약조건
-- 부모테이블 : 참조되고 있는 테이블
-- 자식테이블 : 외래키 제약조건을 선언한 테이블
-- 1. 참조하는 테이블의 컬럼에 존재하지 않는 값을 자식테이블의 외래키 컬럼에 추가x
insert into ddl_emp(emp_name, age, emp_no, dept_code)
values('전정원', 20, '000000-111112', 'D01');
select * from ddl_emp;

-- 2. 자식테이블이 참조하고 있는 행은 삭제할 수 없다.
-- delete from ddl_dept where dept_code = 'D01'; 참조하고 있으므로 삭제 불가

-- 2. 자식테이블이 참조하고 있는 행은 수정 할 수 없다.
-- update ddel_dept set dept_code = 'D04' where dept_code = 'D01'; - 불가

-- 자식테이블이 참조하고 있는 행의 다른 열은 수정 가능
-- update ddl_dept set dept_title = 'dev' where detp_code = 'D01';

-- update ddel_dept set dept_code = 'D11' where dept_code = 'D03'; - 가능
