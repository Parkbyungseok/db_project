...
# emplyoee 테이블에서 이름에 '이'가 포함되고
# 입사일이 01-01-01 이후인 사원의 이름, 사번, 입사일을 조회하기
select emp_name, emp_no, hire_date
from employee
where emp_name like '%이%' and hire_date >= '01-01-01';

# employee 테이블에서 이름이 '연'으로 끝나고 재직중인 사원 조회
select emp_name
from employee
...
# like절의 와일드카드
# _ 문자열의 자리수를 지정
# like '___A%' : 앞자리 3자리뒤에 A가 오는 패턴으로 시작하는,
...
# escape 문자
# escape 'escape 문자'
# 이메일이 '_' 앞에 3글자가 존재하는 패턴의 이메일을 가진 사원의 정보를 조회
select *
from employee
where email like '___%' escape '$';

# ***in***
# 직급코드가 'J7' 또는 'J2'인 사원 중 급여가 200만원 이상인 사원의 이름, 급여, 직급코드 조회
...
# as 별칭
# ifnull(A,B) -> null이 아니라면 A, null 이라면 B
